...
var resizePizzas = function(size) {
  ...
  //Delete useless function determineDx

  function changePizzaSizes(size) {
   //Assign document.getElementsByClassName('randomPizzaContainer') to containerPizza
   var newWidth,
       containerPizza = document.getElementsByClassName('randomPizzaContainer');
   ...
   //Set new width of pizza according to value of newWidth
   for (var i = 0; i < containerPizza.length; i++) {
     containerPizza[i].style.width =  newWidth + '%';
   }
 }
...
var items = document.getElementsByClassName('mover'); // Moves the sliding background pizzas based on scroll position

 function updatePositions() {
   frame++;
   window.performance.mark("mark_start_frame");
   var scrollLocation = document.body.scrollTop / 1250; // No need to assign this value inside for loop
   var halfScreenWidth = ((window.innerWidth > 0) ? window.innerWidth : screen.width) / 2;

   for (var i = 0; i < items.length; i++) {
     var phase = Math.sin(scrollLocation + (i % 5));
     var offset = items[i].basicLeft + 100 * phase - halfScreenWidth; // Adjust the location of pizza with half of screenwidth
     items[i].style.transform = "translateX(" + offset + "px)"; // Change left property with transform 
   }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 6; // Decrease the number of column from 8 to 6
  var s = 256;
  for (var i = 0; i < 18; i++) { // Decrease the number of moving pizza from 200 to 18 which renders on screen
